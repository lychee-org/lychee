...
import { getUserSolvedPuzzleIDs, LOWER_RADIUS, puzzleFromDocument, UPPER_RADIUS } from '../nextPuzzle/nextFor';
...
const similarBatchFor = async (user: User): Promise<Puzzle[]> => {
  ...
  const candidates = (await mongoose.connection.collection('testPuzzles').find({
  ...
  const ret = lastBatch.map((puzzle) => {
    ...
    console.log(`Found ${closest_puzzle.hierarchy_tags} for ${puzzle.hierarchy_tags} with distance ${min_distance}$`)
  ...
  await AllRoundColl.updateOne({ username: user.username }, { $set: { solved: solvedArray  } });
  await LastBatchColl.updateOne({ username: user.username }, { batch: ret }, { upsert: true });
