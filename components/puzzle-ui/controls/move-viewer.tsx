...
const MoveViewer = () => {
  ...
  const renderMoves = (minRows: number) => {
    ...
    let createRow = (i: number, val1: string, val2: string) => {
      return (
        <tr
          className='move-table-row odd:bg-tablerow-dark even:bg-tablerow-light divide-solid divide-x-2 divide-tablerow-divide'
          key={`move-viewer-row-${Math.floor(i / 2)}`}
        >
          <td
            className={cn('moveChild', i === currentMove ? 'highlighted' : '')}
          >
            {val1}
          </td>
          <td
