    ...
    <div className='flex gap-4 items-center'>
      ...
      <form className='grow'>
        <div className="relative">
          ...
          <Input placeholder="Search" className="pl-8 h-fit rounded-lg text-xs border-none bg-secondary py-2" value={filter} onChange={(e) => setFilter(e.target.value)} />
        </div>
      </form>
    </div>
  )
...
function Group({ options, value, onValueChange }: GroupProps) {
  return (
    <RadioGroup value={value} onValueChange={onValueChange} className='flex p-1 gap-1 bg-secondary rounded-lg'>
      {
